...
function HandleState_AETHERIA()
    ...
    if Addons.GetAddon("PurifyItemSelector").Ready then
        ...
        if Addons.GetAddon("PurifyResult").Ready then
            ...
            -- 等待精选完成 - 添加超时机制
            timeout_start = os.clock()
            local purifyTimeout = 120 -- 设置精选超时时间为120秒
            local initialItemCount = Inventory.GetCollectableItemCount(CollectibleItemId, 1)
            
            while CanAetheria() and os.clock() - timeout_start < purifyTimeout do
                -- 检查收藏品数量是否在减少
                local currentItemCount = Inventory.GetCollectableItemCount(CollectibleItemId, 1)
                if currentItemCount < initialItemCount then
                    -- 数量在减少，更新初始数量并重置超时
            ...
            -- 检查是否超时
            if os.clock() - timeout_start >= purifyTimeout then
                DebugLog("精选超时，执行传送重置")
                yield("/echo 精选过程超时，执行传送重置")
                ...
                -- 等待传送完成
                timeout_start = os.clock()
                while Svc.ClientState.TerritoryType ~= 156 and os.clock() - timeout_start < 30 do
                    yield("/wait " .. IntervalRate * 5)
                end
