...
-- 票据兑换设置
ExchangeItemTable = {
    { 4, 8, 7, 1000, 41785 }, -- 橙票用于兑换 (默认为牛票的交换票据)
...
-- 检查是否正在处理收藏品
function IsProcessingCollectable()
    -- 检查是否在收藏品评估界面
    if Addons.GetAddon("CollectableAppraiser") and Addons.GetAddon("CollectableAppraiser").Ready then
        DebugLog("正在处理收藏品")
        return true
    end
    
    -- 检查是否在确认收藏品界面
    if Addons.GetAddon("SelectYesno") and Addons.GetAddon("SelectYesno").Ready then
        DebugLog("正在确认收藏品")
        return true
    end
    
    -- 检查是否有收藏品相关的状态
    if Svc.Condition[38] then -- 38 = Collectable appraisal condition
        DebugLog("正在评估收藏品")
        return true
    end
...
-- 等待收藏品处理完成
function WaitForCollectableProcessing()
    if not IsProcessingCollectable() then
        return false -- 没有在处理收藏品
    end
    ...
    while IsProcessingCollectable() do
        yield("/wait " .. IntervalRate * 5)
...
function FisherStateMachine:State_FISHING()
    ...
    if currentTime - self.lastCheckTime >= self.checkInterval then
        ...
        if NeedsRepair(RepairAmount) or CanExtractMateria() then
            DebugLog("需要修理或精炼，等待收藏品处理完成后停止钓鱼")
            WaitForCollectableProcessing()
        ...
        local currentZone = GetCurrentFishingZone()
        if not IsInZone(currentZone.zoneId) then
            DebugLog("不在钓鱼区域，等待收藏品处理完成后停止钓鱼")
            WaitForCollectableProcessing()
            self:TransitionTo(States.STOP_FISHING)
            return true
        end
